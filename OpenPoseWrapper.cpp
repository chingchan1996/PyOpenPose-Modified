...
cv::Mat OpenPoseWrapper::render(const cv::Mat &rgb)
{
    ...
    const auto poseKeypoints = membersPtr->poseExtractorCaffe.getPoseKeypoints();
    auto poseScore = membersPtr->poseExtractorCaffe.getPoseScores();
    // cv::Mat temp_new_poseKeypoints ;


    /*
    if ( poseKeypoints.size() != 0 ) {
        for ( int i = 0; i < poseKeypoints.getSize(0) ; i ++ ) {

        } // for 

        poseKeypoints.setFrom( temp_new_poseKeypoints ) ;
    } // if 
    else {
        // pass
    }*/

    // return membersPtr->opOutputToCvMat.formatToCvMat( poseKeypoints ) ;
...
OpenPoseWrapper::KeypointGroups OpenPoseWrapper::getKeypoints(KeypointType kpt) {
    ...
    switch(kpt){
        ...
        default: // POSE
            ...
            // res.push_back( membersPtr->opOutputToCvMat.formatToCvMat( persons ) );
            break;
    }
    */
    
    KeypointGroups res;
    op::Array<float> persons = membersPtr->poseExtractorCaffe.getPoseKeypoints();
    res.push_back( persons.getConstCvMat() );
    persons = membersPtr->poseExtractorCaffe.getPoseScores();
    res.push_back( persons.getConstCvMat() ) ;
